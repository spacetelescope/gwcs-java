...
/** Zemax-extended Sellmeier equation with temperature/pressure correction. Input wavelength in microns. */
public class SellmeierZemax implements Transform {
    ...
    private final double deltaCubed;
    private final double nairRefFactor;
    private final double nairObsFactor;
    ...
    public SellmeierZemax(
        ...
        final double temperatureC = temperature - KELVIN_TO_CELSIUS;
        final double refTemperatureC = refTemperature - KELVIN_TO_CELSIUS;
        ...
        this.delta = temperatureC - refTemperatureC;
        this.deltaSquared = delta * delta;
        this.deltaCubed = deltaSquared * delta;
        this.nairRefFactor = refPressure / (1.0 + (refTemperatureC - 15.0) * 3.4785e-3);
        this.nairObsFactor = pressure / (1.0 + (temperatureC - 15.0) * 3.4785e-3);
    }
    ...
    public void evaluate(final double[] inputs, final int inputOffset, final double[] outputs, final int outputOffset) {
        ...
        final double nrefMinusOne = (6432.8
                + 2949810.0 * lam2 / (146.0 * lam2 - 1.0)
                + 5540.0 * lam2 / (41.0 * lam2 - 1.0))
                * 1e-8;

        final double nairObs = 1.0 + nrefMinusOne * nairObsFactor;
        final double nairRef = 1.0 + nrefMinusOne * nairRefFactor;
        ...
        final double lamTkDenom = lamrel2 - lamTk * lamTk;
        final double delnabs = (0.5 * (nrel * nrel - 1.0) / nrel)
                * (d0 * delta
                + d1 * deltaSquared
                + d2 * deltaCubed
                + (e0 * delta + e1 * deltaSquared) / lamTkDenom);
